...
contract P2PEscrow {
    ...
    function autoRelease(uint256 tradeId) external tradeExists(tradeId) {
        ...
        require(block.timestamp > trade.paidAt + 3600, "Release window not reached"); // 1hr after buyer marked paid
    ...
    {
        Trade storage trade = trades[tradeId];
        require(trade.status == TradeStatus.PAID, "Can only dispute paid trades");
