...
describe("P2PEscrow", function () {
  ...
  describe("Rule Changes", function () {
    it("Should not allow dispute before buyer marks paid", async function () {
      await escrow.connect(seller).createTrade(
        buyer.address, await mockUSDT.getAddress(),
        TRADE_AMOUNT, FIAT_AMOUNT, "SDG", "Bank Transfer", DISPUTE_WINDOW
      );
      ...
      await expect(escrow.connect(buyer).openDispute(1))
        .to.be.revertedWith("Can only dispute paid trades");
    });

    it("Should auto-release 1 hour after paidAt", async function () {
      const LONG_WINDOW = 7200;

      await escrow.connect(seller).createTrade(
        buyer.address, await mockUSDT.getAddress(),
        TRADE_AMOUNT, FIAT_AMOUNT, "SDG", "Bank Transfer", LONG_WINDOW
      );

      await escrow.connect(buyer).markAsPaid(1);
      await time.increase(3601);

      await expect(escrow.autoRelease(1)).to.emit(escrow, "TradeCompleted");
